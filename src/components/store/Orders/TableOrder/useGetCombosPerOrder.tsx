...
export const useGetComboPerOrders = (orderIdP?: IOrder['id']) => {
  ...
  const [comboPerOrders, setComboPerOrder] = useState<IComboOrder[] | null>([])
  const [isCustom, setIsCustom] = useState(false)
  const [orderId, setOrderId] = useState(orderIdP)

  useEffect(() => {
    void useComboPerOrders.fetch({ searchBy: { orderId } })
  }, [orderId])

  useEffect(() => {
    if (!useComboPerOrders.data) return
    void Promise.all(useComboPerOrders.data.map(async (comboPerOrder) => {
      return {
        ...comboPerOrder,
  ...
  const getCombosByOrders = async (orderId: IOrder[]): Promise<CombosByOrders> => {
